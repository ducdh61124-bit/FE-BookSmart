...
const MainLayout: React.FC = () => {
    ...
    const menuItems = [{ key: '/books', icon: <BookOutlined />, label: 'Quản lý Kho Sách', onClick: () => navigate('/books') }];
    const userMenuItems = [
    ...
    return (
        <Layout className="min-h-screen">
            <Sider trigger={null} collapsible collapsed={collapsed} theme="dark" width={350} className="shadow-xl">
            ...
            <Layout className="bg-gray-50">
                <Header className="px-6 flex items-center justify-between shadow-sm" style={{ background: colorBgContainer }}>
                    ...
                    <Space size="large" className="flex items-center">
                        <span className="font-medium text-gray-700">Chào, <span className="text-blue-600 font-bold">{displayName}</span>!</span>
                        <Dropdown menu={{ items: userMenuItems }} placement="bottomRight">
                            <Avatar size="large" icon={<UserOutlined />} className="cursor-pointer bg-blue-500 hover:opacity-80 transition-all" />
                        </Dropdown>
                    </Space>
                </Header>
